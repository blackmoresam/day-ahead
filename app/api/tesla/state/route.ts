...
export async function GET(req:Request){
  ...
  const r=await fetch('https://fleet-api.prd.eu.ia.tesla.com/api/1/vehicles',{headers:{Authorization:`Bearer ${decodeURIComponent(m[1])}`,Accept:'application/json'}});
  if(!r.ok)return Response.json({connected:false,error:'Tesla vehicle data unavailable'},{status:502});
